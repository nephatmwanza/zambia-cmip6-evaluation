...
#
# Usage (all arguments optional; defaults reproduce the rainfall aggregation):
#   scripts/04_aggregate_provinces.sh [INDEX_NC] [MASK_NC] [OUT_CSV] [YEAR_COLUMN] [VARS...]
#
# Examples:
#   scripts/04_aggregate_provinces.sh                       # rainfall, ONDJFM (default)
#   scripts/04_aggregate_provinces.sh IN.nc MASK.nc OUT.csv year v1 v2 ...
#
set -euo pipefail
cd "$(dirname "$0")/.."

IDX=${1:-data/processed/zambia_indices_ondjfm.nc}
MASK=${2:-data/processed/zambia_province_mask.nc}
OUT_CSV=${3:-data/processed/zambia_indices_ondjfm_by_province.csv}
YEAR_COL=${4:-season_start_year}
shift 4 2>/dev/null || true
VARS=${*:-"cdd cwd r95p r99p r95pd r99pd prcptot r1mm"}
